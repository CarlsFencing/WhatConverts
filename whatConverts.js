    ...
    function onRequest(context) {
        if (context.request.method === 'GET') {
            try {
                ...
                log.debug({
                    title: 'Lead Details with Keywords',
                    details: JSON.stringify(customerData, null, 2)
                });

                context.response.write({
                    output: JSON.stringify(customerData.slice(0, 100), null, 2) // Only write up to 100 leads
                });
